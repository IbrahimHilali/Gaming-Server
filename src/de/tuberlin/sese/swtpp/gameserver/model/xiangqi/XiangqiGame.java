...
public class XiangqiGame extends Game implements Serializable {
	...
	public boolean tryMove(String moveString, Player player) {
		// TODO: implement
		if(!moveString.matches("^[a-i][0-9]-[a-i][0-9]$")) {
			return false;
		}
		if( this.isRedNext() !=  this.board.isRedMove(moveString) ) {
			return false;
		}
		if (this.board.checkMove(moveString)) {
			Move move = new Move(moveString, this.board.toString(), player);
			this.history.add(move);
			this.board.update(move);
			move.setBoard(this.getBoard());
			if (this.board.checkMate(move, this.isRedNext())) {
				player.finishGame();
				return this.regularGameEnd(player);
			}
			this.setNextPlayer(this.isRedNext() ? this.blackPlayer : this.redPlayer);
			return true;
		}
		return false;
	}
